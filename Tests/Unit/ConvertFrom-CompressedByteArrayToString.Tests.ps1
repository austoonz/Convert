    ...
    foreach ($encoding in $encodings)
    {
        Context -Name $encoding.Encoding -Fixture {
            ...
            It -Name 'Supports ErrorActionPreference Continue' -Test {
                ...
                $assertion.Exception.Message | Should -BeExactly 'Exception calling "CopyTo" with "1" argument(s): "The magic number in GZip header is not correct. Make sure you are passing in a GZip stream."'
            }
        }
    }
}
