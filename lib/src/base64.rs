...
///
/// This is a lenient version of `base64_to_string` that automatically falls back to
/// Latin-1 (ISO-8859-1) encoding when the decoded bytes are invalid for the specified
/// encoding. This is useful for handling binary data encoded as Base64.
///
/// # Safety
...
mod tests {
    ...
    fn test_base64_to_string_happy_path_utf8() {
        ...
        let result = unsafe { base64_to_string(input.as_ptr(), encoding.as_ptr()) };
    ...
    fn test_base64_to_string_null_input_pointer() {
        ...
        let result = unsafe { base64_to_string(std::ptr::null(), encoding.as_ptr()) };
    ...
    fn test_base64_to_string_null_encoding_pointer() {
        ...
        let result = unsafe { base64_to_string(input.as_ptr(), std::ptr::null()) };
    ...
    fn test_base64_to_string_round_trip() {
        ...
        // Encode
        let encoded_ptr = unsafe { string_to_base64(input.as_ptr(), encoding.as_ptr()) };
        assert!(!encoded_ptr.is_null(), "Encoding should succeed");

        // Decode
        let decoded_ptr = unsafe { base64_to_string(encoded_ptr, encoding.as_ptr()) };
        ...
        assert_eq!(
            ...
            "Round-trip should preserve original string"
        );
    ...
    fn test_base64_to_string_various_encodings() {
        ...
        for (base64_input, enc, expected) in test_cases {
            ...
            if !result.is_null() {
                ...
                assert!(
                    !result_str.is_empty() || expected.is_empty(),
                    "Result should not be empty for encoding: {}",
                    enc
                );
                unsafe { crate::memory::free_string(result) };
            }
    ...
    fn test_bytes_to_base64_happy_path() {
        ...
        assert_eq!(
            result_str, "SGVsbG8=",
            "Encoded bytes should produce 'SGVsbG8='"
        );
        unsafe { crate::memory::free_string(result) };
    }
    ...
    fn test_bytes_to_base64_zero_length() {
        ...
        assert!(
            !result.is_null(),
            "Result should not be null for zero length"
        );
        let result_str = unsafe { CStr::from_ptr(result).to_str().unwrap() };
    ...
    fn test_bytes_to_base64_large_byte_array() {
        ...
        let result = unsafe { bytes_to_base64(large_bytes.as_ptr(), large_bytes.len()) };

        assert!(
            !result.is_null(),
            "Result should not be null for large byte array"
        );
        ...
        assert!(
            !result_str.is_empty(),
            "Result should not be empty for 1MB input"
        );

        // Base64 encoding increases size by ~33%, so 1MB should produce ~1.33MB
        assert!(
            result_str.len() > 1_000_000,
            "Encoded result should be larger than 1MB"
        );
    ...
    fn test_bytes_to_base64_various_byte_patterns() {
        ...
        for (bytes, expected) in test_cases {
            ...
            assert!(
                !result.is_null(),
                "Result should not be null for byte pattern: {:?}",
                bytes
            );
            let result_str = unsafe { CStr::from_ptr(result).to_str().unwrap() };
            assert_eq!(
                result_str, expected,
                "Byte pattern {:?} should encode to '{}'",
                bytes, expected
            );
    ...
    fn test_bytes_to_base64_binary_data() {
        ...
        let result = unsafe { bytes_to_base64(binary_data.as_ptr(), binary_data.len()) };

        assert!(
            !result.is_null(),
            "Result should not be null for binary data"
        );
        let result_str = unsafe { CStr::from_ptr(result).to_str().unwrap() };
        assert_eq!(
            result_str, "iVBORw0KGgo=",
            "PNG header should encode correctly"
        );
    ...
    fn test_bytes_to_base64_empty_array() {
        ...
        let result = unsafe { bytes_to_base64(empty_bytes.as_ptr(), 0) };

        assert!(
            !result.is_null(),
            "Result should not be null for empty array"
        );
        let result_str = unsafe { CStr::from_ptr(result).to_str().unwrap() };
    ...
    fn test_bytes_to_base64_round_trip_with_string_to_base64() {
        ...
        let result_bytes = unsafe { bytes_to_base64(bytes.as_ptr(), bytes.len()) };
        ...
        let encoding = CString::new("UTF8").unwrap();
        let result_string = unsafe { string_to_base64(text_cstring.as_ptr(), encoding.as_ptr()) };
    ...
    fn test_bytes_to_base64_all_byte_values() {
        ...
        let result = unsafe { bytes_to_base64(all_bytes.as_ptr(), all_bytes.len()) };

        assert!(
            !result.is_null(),
            "Result should not be null for all byte values"
        );
        let result_str = unsafe { CStr::from_ptr(result).to_str().unwrap() };
        ...
        for ch in result_str.chars() {
            assert!(
                valid_base64_chars.contains(ch),
                "Result should only contain valid Base64 characters, found: {}",
                ch
            );
        }
    ...
    fn test_base64_to_bytes_happy_path() {
        ...
        assert_eq!(
            byte_slice,
            &[72, 101, 108, 108, 111],
            "Decoded bytes should be [72, 101, 108, 108, 111] (Hello)"
        );
    ...
    fn test_base64_to_bytes_invalid_base64() {
        ...
        assert_eq!(
            out_length, 0,
            "Output length should be 0 for invalid Base64"
        );
    }
    ...
    fn test_base64_to_bytes_binary_data() {
        ...
        assert_eq!(
            out_length, 8,
            "Output length should be 8 bytes for PNG header"
        );
        ...
        assert_eq!(
            byte_slice,
            expected_png_header.as_slice(),
            "Decoded bytes should match PNG header"
        );
    ...
    fn test_base64_to_bytes_round_trip() {
        ...
        assert_eq!(
            ...
            "Round-trip should preserve original bytes"
        );
    ...
    fn test_base64_to_bytes_large_data() {
        ...
        let encoded_ptr = unsafe { bytes_to_base64(large_bytes.as_ptr(), large_bytes.len()) };
        assert!(!encoded_ptr.is_null(), "Encoding should succeed");
        ...
        assert!(
            !decoded_ptr.is_null(),
            "Decoding should succeed for large data"
        );
    ...
    fn test_base64_to_bytes_all_byte_values() {
        ...
        // Encode
        let encoded_ptr = unsafe { bytes_to_base64(all_bytes.as_ptr(), all_bytes.len()) };
        assert!(!encoded_ptr.is_null(), "Encoding should succeed");
        ...
        assert!(
            !decoded_ptr.is_null(),
            "Decoding should succeed for all byte values"
        );
    ...
    fn test_base64_to_bytes_various_lengths() {
        ...
        for (base64_input, expected_bytes) in test_cases {
            ...
            assert_eq!(
                ...
                "Decoded bytes should match expected for input '{}'",
                base64_input
            );
    ...
    fn test_base64_to_bytes_null_output_length_pointer() {
        ...
        let result = unsafe { base64_to_bytes(input.as_ptr(), std::ptr::null_mut()) };
    ...
    fn test_encoding_case_insensitivity_performance() {
        ...
        let encoding_variants = vec![
            "utf8", "UTF8", "Utf8", "uTf8", "ascii", "ASCII", "Ascii", "unicode", "UNICODE",
            "Unicode",
        ];

        for encoding in encoding_variants {
            let enc_cstring = CString::new(encoding).unwrap();
            let result = unsafe { string_to_base64(input.as_ptr(), enc_cstring.as_ptr()) };
    ...
    fn test_encoding_name_with_hyphens() {
        ...
        for (hyphenated, non_hyphenated) in encoding_variants {
            ...
            let result1 = unsafe { string_to_base64(input.as_ptr(), enc1.as_ptr()) };
            let result2 = unsafe { string_to_base64(input.as_ptr(), enc2.as_ptr()) };
    ...
    fn test_utf7_rejection_is_documented() {
        ...
        for variant in utf7_variants {
            ...
            assert!(
                result.is_null(),
                "UTF7 variant '{}' should be rejected",
                variant
            );

            // Verify error message is set
            let error = unsafe { crate::error::get_last_error() };
            assert!(
                !error.is_null(),
                "Error message should be set for UTF7 variant '{}'",
                variant
            );
            ...
            assert!(
                error_str.contains("UTF7") || error_str.contains("deprecated"),
    ...
    fn test_ascii_encoding_rejects_non_ascii() {
        ...
        for test_str in non_ascii_strings {
            ...
            let error_str = unsafe { CStr::from_ptr(error).to_str().unwrap() };
            assert!(
                error_str.contains("ASCII") || error_str.contains("non-ASCII"),
                "Error should mention ASCII issue, got: {}",
                error_str
            );
    ...
    fn test_ascii_encoding_accepts_valid_ascii() {
        ...
        for test_str in ascii_strings {
            ...
            assert!(
                !result.is_null(),
                "ASCII encoding should accept valid ASCII string: {}",
                test_str
            );
    ...
    fn test_utf16_byte_order() {
        ...
        let mut out_length: usize = 0;
        let bytes_ptr = unsafe { base64_to_bytes(result, &mut out_length as *mut usize) };
    ...
    fn test_utf32_encoding_size() {
        ...
        assert_eq!(
            out_length, 8,
            "UTF-32 'AB' should be 8 bytes (2 chars × 4 bytes)"
        );
    ...
    fn test_default_encoding_is_utf8() {
        ...
        let result_utf8 = unsafe { string_to_base64(input.as_ptr(), utf8_enc.as_ptr()) };
        let result_default = unsafe { string_to_base64(input.as_ptr(), default_enc.as_ptr()) };
        ...
        let handles: Vec<_> = (0..10)
            .map(|i| {
                thread::spawn(move || {
                    ...
                    // Decode
                    let decoded = unsafe { base64_to_string(result, encoding.as_ptr()) };
                    assert!(
                        !decoded.is_null(),
                        "Decoding should succeed in thread {}",
                        i
                    );
    ...
    fn test_concurrent_error_isolation() {
            ...
            .map(|i| {
                ...
                thread::spawn(move || {
                    ...
                    if i % 2 == 0 {
                        ...
                        let encoding = CString::new("UTF8").unwrap();
                        let result =
                            unsafe { string_to_base64(std::ptr::null(), encoding.as_ptr()) };
        ...
        assert!(
            ...
            "All threads should handle errors correctly"
        );
    }

    // ========== Tests for Latin-1 encoding and fallback ==========

    #[test]
    fn test_latin1_encoding_direct() {
        // Test: Latin-1 encoding should handle any byte value (0x00-0xFF)
        // Note: Null byte (0x00) is replaced with replacement character for C string safety
        let all_bytes: Vec<u8> = (0..=255).collect();
        ...
        // The string has 256 Unicode code points (chars), but may have more bytes
        // because characters > 127 are multi-byte in UTF-8
        assert_eq!(s.chars().count(), 256, "Result should have 256 characters");

        // Verify mapping: each byte should map to its corresponding Unicode code point
        // except for null byte (0x00) which maps to replacement character (U+FFFD)
    ...
    fn test_latin1_encoding_variants() {
        // Test: all Latin-1 encoding name variants should work
    ...
    fn test_convert_bytes_to_string_with_fallback_utf8_success() {
        // Test: valid UTF-8 should succeed without fallback
    ...
    fn test_convert_bytes_to_string_with_fallback_invalid_utf8() {
        // Test: invalid UTF-8 should fall back to Latin-1
        // Note: null bytes (0x00) are replaced with replacement character
        ...
        // Verify the string can be converted back to the original bytes via Latin-1
    ...
    fn test_convert_bytes_to_string_with_fallback_non_ascii() {
        // Test: non-ASCII bytes with ASCII encoding should fall back to Latin-1
        let non_ascii: Vec<u8> = vec![72, 200, 111]; // 'H', 0xC8, 'o'
    ...
    fn test_convert_bytes_to_string_with_fallback_structural_errors_fallback() {
        // Test: structural errors (wrong byte length for UTF-16) should also fall back to Latin-1
        // This is the desired behavior - binary data may have any length
        // Note: null bytes are replaced with replacement character, so we use non-null bytes
        let odd_bytes: Vec<u8> = vec![72, 65, 101]; // Odd length for UTF-16, no null bytes
    ...
    fn test_convert_bytes_to_string_with_fallback_unsupported_encoding() {
        // Test: unsupported encoding should propagate error
    ...
    fn test_convert_bytes_to_string_with_fallback_binary_data_round_trip() {
        ...
        // This simulates the Get-SecureBootUEFI scenario from issue #24
        // Note: null bytes (0x00) are replaced with replacement character, so we exclude them
        let binary_data: Vec<u8> = vec![
            0x89, 0x50, 0x4E, 0x47, // PNG-like header
            0x0D, 0x0A, 0x1A, 0x0A, 0xFF, 0xFE, 0x80, // Various binary bytes (no null)
        ];
    ...
    fn test_convert_bytes_to_string_with_fallback_null_bytes_replaced() {
        // Test: null bytes are replaced with Unicode replacement character
